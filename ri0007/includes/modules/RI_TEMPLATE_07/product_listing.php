...
if(!isset($PRODUCT_LISTING_COLUMNS_PER_ROW))
	$PRODUCT_LISTING_COLUMNS_PER_ROW = PRODUCT_LISTING_COLUMNS_PER_ROW;
...
if($is_index_listing){
	...
	$listing_split = new singleListing($listing_sql, $max_index_listing);
}
else{
    ...
    for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
      ...
      switch ($column_list[$col]) {
        ...
        if (isset($_GET['manufacturers_id'])) {
          $lc_text = '<div class="itemTitle"><a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '">' . zen_trunc_string($listing->fields['products_name'], '40', false) . '</a></div><div class="listingDescription">' . zen_trunc_string(zen_clean_html(stripslashes(zen_get_products_description($listing->fields['products_id'], $_SESSION['languages_id']))), PRODUCT_LIST_DESCRIPTION) . '</div>' ;
        } else {
          $lc_text = '<div class="itemTitle"><a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '">' . zen_trunc_string($listing->fields['products_name'], '40', true) . '</a></div><div class="listingDescription">' . zen_trunc_string(zen_clean_html(stripslashes(zen_get_products_description($listing->fields['products_id'], $_SESSION['languages_id']))), PRODUCT_LIST_DESCRIPTION) . '</div>';
        }
        break;
        case 'PRODUCT_LIST_MANUFACTURER':
        $lc_align = '';
        ...
        break;
        case 'PRODUCT_LIST_PRICE':
        $lc_price = '<div class="itemPrice">'.ri_get_products_display_price($listing->fields['products_id']) . '</div>';
        $lc_align = 'right';
        $lc_text =  $lc_price;
        ...
        if (zen_has_product_attributes($listing->fields['products_id']) or PRODUCT_LIST_PRICE_BUY_NOW == '0') {
          $lc_button = '<a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? $_GET['cPath'] : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '">' . zen_image_button(BUTTON_IMAGE_MORE_INFO, BUTTON_MORE_INFO_ALT) . '</a>';
        } else {
...
// qty box with add to cart button
        ...
        $the_button = '<div class="itemCartAdd">'.$lc_button.'</div>';
        $products_link = '<a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . ( ($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ? zen_get_generated_category_path_rev($_GET['filter_id']) : $_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id'])) . '&products_id=' . $listing->fields['products_id']) . '">' . zen_image_button(BUTTON_IMAGE_MORE_INFO, BUTTON_MORE_INFO_ALT) . '</a>';
        $lc_text .= (zen_get_show_product_switch($listing->fields['products_id'], 'ALWAYS_FREE_SHIPPING_IMAGE_SWITCH') ? (zen_get_product_is_always_free_shipping($listing->fields['products_id']) ? '<div class="freeShippingIcon">' . TEXT_PRODUCT_FREE_SHIPPING_ICON . '</div>' : '') : '');
		$lc_text .= ri_get_products_quantity_min_units_display($listing->fields['products_id']) . zen_get_buy_now_button($listing->fields['products_id'], $the_button, $products_link);
    ...
    if (PRODUCT_LISTING_LAYOUT_STYLE == 'columns') {
      ...
      $list_box_contents[$rows][$column] = array('params' => 'class="centerBoxContentsProducts back"' . ' ' . 'style="width:' . ($col_width - 1) . '%;"',
                                                 'text'  => $lc_text);
      $column ++;
